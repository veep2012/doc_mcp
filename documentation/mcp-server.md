...
## Change Log
- 2026-09-06 | v4.4 | Documented bounded, deterministic MCP `resources/list` pagination and safe opaque cursor validation, including bounded keyset retrieval, compact catalog revision derivation, and the catalog-first ordering required for non-paginating clients.
...
`resources/list` returns the readable `docmcp://sites` catalog resource first, followed by configured site resources and indexed page resources in deterministic URI order. Responses contain at most 100 resources. The server retrieves only a bounded keyset window around the requested page and derives the opaque cursor revision from compact per-index metadata rather than materializing the full resource catalog. When more resources remain, the response includes an opaque `nextCursor`; pass that value as the `cursor` request parameter to retrieve the next page. Clients that omit `cursor` receive the first page, preserving standard MCP discovery behavior. When no documentation sites are configured, the response still contains the readable `docmcp://sites` catalog resource and no `nextCursor`.
