...
## Change Log
- 2026-09-06 | v2.10 | Added protocol-level `resources/list` pagination coverage, including deterministic continuation, complete discovery, invalid or expired cursor rejection, packaged-harness continuation coverage for contract version 1.2, and empty-catalog and stale-cursor coverage across the stdio boundary. The harness corpus validator now requires the cursor-free request and `$previous.nextCursor` continuation pair instead of treating pagination coverage as optional. Resource discovery now retrieves bounded index windows and derives cursor revisions from compact index metadata.
...
- `TS-TF-013` - The MCP version-comparison harness validates its fixture and safe configuration, requires a cursor-free `resources/list` request immediately followed by a `$previous.nextCursor` continuation request, runs both protocol-level resource-list pages, resource-template discovery, resource reads for the catalog (`docmcp://sites`), a configured site (`docmcp://site/<site-id>`), and an indexed page (`docmcp://site/<site-id>/page/<page-key>`), `get_version`, representative search, and missing-site corpus, requires public contract version `1.2`, rejects any baseline/current `contract_version` mismatch even when package versions are allowlisted, normalizes only allowlisted fields (including JSON-encoded `get_version` payload versions), and reports unexpected differences.
...
- `TS-TF-026` - MCP `resources/list` returns a bounded, deterministic catalog of the site and indexed-page resources with an opaque protocol cursor, rejects stale cursors, and has no duplicate or omitted entries across continuation requests.
- `TS-TF-028` - MCP `resources/list` returns the public catalog resource without continuation when no documentation sites are configured.
- `TS-TF-027` - `make test` completes without warnings from repository tests or the MCP dependency initialization path.
...
- Expected Result: Initialization advertises the `resources` capability; site IDs use trimmed NFC names encoded as one UTF-8 URI segment, the server advertises `docmcp://site/{site_id}` and `docmcp://site/{site_id}/page/{page_key}` as resource templates, and the presence of the page template indicates addressable child page resources. Colliding canonical names fail configuration loading, catalog and site resources disclose no runtime paths or credentials, site resources return only the site name, page count, crawl/index status, page URI template, and instructions to use `list_pages` for the page catalog, every `list_pages` `resource_uri` can be read through standard resource access to return the page Markdown and the corresponding page remains available through `fetch_page`, page keys canonicalize URLs before lookup, pages outside the configured host/start-path scope are rejected, malformed or missing resources return safe protocol errors, and each `search_docs` result has its deterministic page `resource_uri`. URL writes use the same canonical identity, while legacy rows differing by NFC form, scheme/host casing, fragment, or removable trailing slash are resolved by canonical identity so every emitted page resource URI remains readable. For `auth_required` sites, authentication is established before indexing and the configured index is readable by the local MCP server; no per-caller authorization is claimed.
...
- Action: Call `resources/list` without a cursor, continue with each returned `nextCursor`, repeat the initial request, mutate the indexed catalog, and submit malformed, incompatible, and stale cursors through stdio.
- Expected Result: Each response contains at most the fixed server page size of resources, with `docmcp://sites` first for backward compatibility and all remaining site/page resources in deterministic URI order; index retrieval is bounded around the requested window rather than materializing the full page catalog, and cursor revisions are derived from compact index metadata. `nextCursor` is present only while additional resources remain. Continuation requests return every catalog, site, and indexed-page resource exactly once with no duplicates or omissions. Repeated initial requests preserve order. A malformed, incompatible, or stale cursor returns the MCP `Invalid params` protocol error without internal details; server debug logs classify rejected cursors as malformed, revision-mismatched, or missing-endpoint without logging cursor values or catalog contents, and observability events record safe pagination-usage fields for served pages and rejection categories. `resources/templates/list` remains available and continues to advertise the site and page templates.

#### TS-TF-028
- Purpose: Validate empty MCP protocol-level resource discovery.
- Preconditions: The server has no configured documentation sites.
- Action: Call `resources/list` without a cursor.
- Expected Result: The server returns exactly the readable `docmcp://sites` catalog resource without a `nextCursor`, and the request remains a valid successful MCP response.
...
#### TS-TF-027
- Purpose: Keep the canonical test command clean so warnings identify actionable regressions.
- Preconditions: The development virtual environment contains the pinned test dependencies.
- Action: Run `make test`, including unit and smoke suites; exercise the missing-browser authentication test that rejects an async operation.
- Expected Result: All selected tests pass and pytest reports no warnings. Rejected asynchronous operations are explicitly closed when a test double interrupts their runner.
...
- `TS-TF-028` -> `tests/test_tools.py::test_mcp_resource_list_empty_catalog`
- `TS-TF-027` -> `tests/test_auth_cli.py::test_auth_cli_reports_missing_browser`, `tests/test_smoke_support.py::test_make_test_declares_unit_before_smoke`, `Makefile`
