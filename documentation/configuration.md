...
- PDF handling has no site-specific configuration key. The crawler automatically recognizes `.pdf` URLs and responses with `Content-Type: application/pdf`; PDFs selected through `--pages` or `--pages-file` follow the same behavior. PDF downloads use `playwright.launch.proxy` and the browser context's saved authentication state.
